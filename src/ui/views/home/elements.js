...
const Elements = (root = document) => ({
  ...
  get main_completed_tasks() {
    return must(
      root.querySelector(".main-drop-down-task"),
      "Seção principal de tarefas concluídas"
    );
  },
  get completed_tasks() {
    return must(
      root.querySelector(".drop-down-task"),
  ...
  get button_completed_tasks() {
    return must(
      root.getElementById("button-completed-tasks"),
      "Botão de exibir tarefas concluídas"
    );
  },
  get task_checkbox() {
  ...
  get settings_drop_down() {
    return must(
      root.querySelector(".settings-drop-down"),
